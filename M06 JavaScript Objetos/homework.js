...
function crearGato(nombre, edad) {
  ...
  var gato = {
    nombre: nombre,
    edad: edad,
    meow: function retornarMeow(prop) {
      return "Meow!";
    },
  };
  return gato;
...
function tienePropiedad(objeto, propiedad) {
  ...
  // En ese caso retornar true. Caso contrario, false.
  // Tu código:
  var verificarPropiedad = objeto.hasOwnProperty(propiedad);
  if (verificarPropiedad) return true;
  else return false;
}
...
function sumarLikesDeUsuario(objetoUsuario) {
  ...
  var arrayPosts = objetoUsuario.posts;
  for (let i = 0; i < arrayPosts.length; i++) {
    sumarLikesDeUsuario += arrayPosts[i]["likes"];
  }
...
function agregarMetodoCalculoDescuento(objetoProducto) {
  ...
  objetoProducto.calcularPrecioDescuento = () => {
    var multiplicarPrecio =
      objetoProducto.precio * objetoProducto.porcentajeDeDescuento;
    var precioFinal = objetoProducto.precio - multiplicarPrecio;
    return precioFinal;
  };
